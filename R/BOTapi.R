...
check.define <- function(){
  ...
  if(!exists("id")){message('Please enter the information on your ID. If you do not have it yet, Please visit https://apiportal.bot.or.th/bot/public/')}
}
...
code.status.warn <- function(resp){
  ...
  if(code != 200){
    massage(past0('Respond Code ', code ,' Request Unsuccess',))
  }
}
...
#'
#' Get weighted-average international bank exchange rate  \cr 
...
#'
#' Get series of financial institutions' holidays in given year \cr 
#' 
#' @param id Client ID obtained from BoT
